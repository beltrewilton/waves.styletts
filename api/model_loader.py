...
# load phonemizer
import phonemizer # PHONEMIZER_ESPEAK_LIBRARY=/opt/homebrew/Cellar/espeak/1.48.04_1/lib/libespeak.dylib
...
ASR_path = f"../StyleTTS2/{ASR_path}"
ASR_config = f"../StyleTTS2/{ASR_config}"
text_aligner = load_ASR_models(ASR_path, ASR_config)

# load pretrained F0 model
# Fundamental frequency (F0)
F0_path = config.get('F0_path', False)
F0_path = f"../StyleTTS2/{F0_path}"
pitch_extractor = load_F0_models(F0_path)

# load BERT model
from Utils.PLBERT.util import load_plbert
BERT_path = config.get('PLBERT_dir', False)
BERT_path = f"../StyleTTS2/{BERT_path}"
...
params_whole = torch.load("../StyleTTS2/Models/LibriTTS/epochs_2nd_00020.pth", map_location='cpu')
...
def inference(text, ref_s, alpha = 0.3, beta = 0.7, diffusion_steps=5, embedding_scale=1, speed=1):
    ...
    return out.squeeze().cpu().numpy()[..., :-50] #TODO weird pulse at the end of the model, need to be fixed later
...
if __name__ == "__main__":
    ...
    # reference_dicts['1789_142896'] = "../StyleTTS2/Demo/reference_audio/1789_142896_000022_000005.wav"
    ...
    for k, path in reference_dicts.items():
        ...
        start = time.time()
        wav = inference(text, ref_s, alpha=0.3, beta=0.7, diffusion_steps=20, embedding_scale=3)
        rtf = (time.time() - start) / (len(wav) / 24000)
        synth_wav_file = o = f"{path[:-4]}_synth_embedding_scale_3.wav"
        save_wav(synth_wav_file, wav, 24_000)
        print(f"RTF = {rtf:5f}")
        # import IPython.display as ipd
        print(k + ' Synthesized:')
        os.system(f'ffplay {synth_wav_file} -autoexit')
        print('Reference:')
